    ...
    view! { 
        <div class="w-full fixed z-50 h-20 shadow-sm flex items-center justify-between px-8 font-light transition-all bg-white/90 backdrop-blur-md">
            ...
            <div class="flex items-center space-x-2">
                <a href="/"><img src="/public/img/fueldao.svg" alt="Fuel DAO Logo" class="h-8"/></a>
            </div>
...
fn UserPrincipal() -> impl IntoView {
    ...
    let principal = move || {
      auth_client.get().map(|f|  {
        match f.is_authenticated() {
    ...
    view! {
        <Show when=move || principal().is_some() fallback=move|| view! {
            <button on:click=move |_| { match login() {
