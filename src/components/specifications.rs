...
#[component]
pub fn SpecificationComponent() -> impl IntoView {
    let metadata = CollectionMetadata::default();
    // Default values for optional fields
    let description = metadata
        .description
        .unwrap_or_else(|| "No description provided".to_string());
    let weight = metadata.weight.unwrap_or_else(|| "--".to_string());
    let drive_type = metadata
        .drive_type
        .unwrap_or_else(|| "Electric".to_string());
    let displays = metadata.displays.unwrap_or_else(|| "N/A".to_string());
    let seating = metadata.seating.unwrap_or_else(|| "5 Seater".to_string());
    let cargo = metadata.cargo.unwrap_or_else(|| "--".to_string());
    let overall_height = metadata.overall_height.unwrap_or_else(|| "--".to_string());
    let overall_width = metadata.overall_width.unwrap_or_else(|| "--".to_string());
    let overall_length = metadata.overall_length.unwrap_or_else(|| "--".to_string());
    let track_rear = metadata.track_rear.unwrap_or_else(|| "--".to_string());
    let track_front = metadata.track_front.unwrap_or_else(|| "--".to_string());
    let ground_clearance = metadata
        .ground_clearance
        .unwrap_or_else(|| "N/A".to_string());
    let key_features = metadata
        .key_features
        .unwrap_or_else(|| vec!["Tesla Autopilot".to_string(), "0-60 in 3.2s".to_string()]);
    let range_per_charge = metadata
        .range_per_charge
        .unwrap_or_else(|| "--".to_string());
    let acceleration = metadata
        .acceleration
        .unwrap_or_else(|| "0-60 in 3.2s".to_string());
    let charging_speed = metadata.charging_speed.unwrap_or_else(|| "400".to_string());
    let wheels = metadata.wheels.unwrap_or_else(|| "18\"".to_string());
    let purchase_price = metadata
        .purchase_price
        .map(|p| format!("€ {}", p.to_string()))
        .unwrap_or_else(|| "--".to_string());
    let brochure_url = metadata.brochure_url.clone();
    let battery = metadata.battery.unwrap_or_else(|| "-- KwH".to_string());
    ...
    view! {
        <div class="flex flex-col gap-8 py-4">
            <div class="grid grid-cols-3 gap-4">
                ...
                <SpecDetail title="Wheels" value=wheels />
                ...
                <div class="flex flex-col">
                    ...
                    {brochure_url
                        .map(|url| {
                            view! {
                                <div>
                                    <a target="_blank" href=url class="font-light">
                                        "Link"
                                    </a>
                                </div>
                            }
                        })
                        .unwrap_or_else(|| view! { <div class="font-light">"N/A"</div> })}
                </div>
