...
use crate::{
    canister::token::{GetMetadataRet, SaleStatus},
    outbound::collection_canister_calls::{CollectionData, CollectionId},
};
...
impl SaleStatusState {
    ...
    pub fn get_listing_status(canister: Principal) -> impl Fn() -> SaleStatus {
        move || {
            Self::get()
                    ...
                    .get()
                    .get(&canister)
                    .unwrap_or(&SaleStatus::Rejected).clone()
                
        }
    }

    pub fn set_listing_satatus(canister: Principal, status: SaleStatus) {
        Self::get().sale_status.update(|f| {
            f.insert(canister, status);
        });
    }
    ...
    pub fn set_listing_metadata(canister: Principal, metadata: GetMetadataRet) {
        Self::get().metadata.update(|f| {
            f.insert(canister, Some(metadata));
        });
    }
