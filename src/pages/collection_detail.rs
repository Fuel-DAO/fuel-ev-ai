...
use crate::components::admin::invest_info_admin::{
    AddFallbackPrincipalForAnnonymousInvestor, ConcludeSaleAdminComponent, RefundICPsToAnnonymous,
    TransferAmountFromAnnonymousToInvestor,
};
        ...
        move |token_id| {
            send_wrap(async move {
                ...
                if let Some(cans_rc) = Canisters::get() {
                    let metadata =
                        get_collection_metadata_from_token_canister(&cans_rc, principal).await;
                        ...
                        Ok(meta) => match meta {
                            crate::canister::token::Result4::Ok(get_metadata_ret) => {
                                SaleStatusState::set_listing_metadata(
                                    token_canister_id,
                                    get_metadata_ret.clone(),
                                );
                                SaleStatusState::set_listing_satatus(
                                    token_canister_id,
                                    status.clone().unwrap_or(SaleStatus::Rejected),
                                );
...
fn CarDetailPage(metadata: GetMetadataRet, status: Option<SaleStatus>) -> impl IntoView {
    let params: Memo<Result<CollectionParams, leptos_router::params::ParamsError>> =
        use_params::<CollectionParams>();
